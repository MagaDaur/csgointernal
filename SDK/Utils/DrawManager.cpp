...
CD3DFont* DrawManager::RenderCreateFont(const TCHAR* name, float size, int width)
{
    CD3DFont* font = new CD3DFont(name, size, width);
    HRESULT hr;
    hr = font->InitDeviceObjects(pDevice);
...
void DrawManager::InitDeviceObjects(LPDIRECT3DDEVICE9EX pDevice)
{
	...
	g_Fonts.pFontTahoma20 = std::make_unique<CD3DFont>(L"Square Sans Serif 7", 20, FW_MEDIUM);
	g_Fonts.pFontTahoma30 = std::make_unique<CD3DFont>(L"Square Sans Serif 7", 30, FW_MEDIUM);
	g_Fonts.pFontMenu2      = std::make_unique<CD3DFont>(L"Tahoma", 10, FW_MEDIUM);
	g_Fonts.pFontMenu       = std::make_unique<CD3DFont>(L"Tahoma", 8, FW_NORMAL);
    g_Fonts.pFontCorbel = std::make_unique<CD3DFont>(L"Verdana", 15, FW_EXTRABOLD);
...
void DrawManager::RestoreDeviceObjects(LPDIRECT3DDEVICE9EX pDevice)
{
    this->pDevice = pDevice;
    this->pDevice->GetViewport(&pViewPort); 
